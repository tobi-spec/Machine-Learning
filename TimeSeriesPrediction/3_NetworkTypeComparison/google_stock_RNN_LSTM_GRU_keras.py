...
figure, axis = plt.subplots(3, figsize=(18, 12), sharex=True, sharey=True)
figure.suptitle('Model Predictions')

# Plot for RNN predictions
axis[0].plot(train.index[150:], train.Open[150:], label="train_data", color="b")
axis[0].plot(test.index, test.Open, label="test_data", color="g")
axis[0].plot(test.index[50:], y_RNN_O, label="y_RNN", color="brown")
axis[0].legend()
axis[0].title.set_text("Basic RNN")

# Plot for LSTM predictions
axis[1].plot(train.index[150:], train.Open[150:], label="train_data", color="b")
axis[1].plot(test.index, test.Open, label="test_data", color="g")
axis[1].plot(test.index[50:], y_LSTM_O, label="y_LSTM", color="orange")
axis[1].legend()
axis[1].title.set_text("LSTM")

# Plot for GRU predictions
axis[2].plot(train.index[150:], train.Open[150:], label="train_data", color="b")
axis[2].plot(test.index, test.Open, label="test_data", color="g")
axis[2].plot(test.index[50:], y_GRU_O, label="y_GRU", color="red")
axis[2].legend()
axis[2].title.set_text("GRU")
